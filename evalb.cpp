...
typedef struct ss_terminal {
    ...
    void show() {
        printf("ss_terminal.word:%s\n",word);
        printf("ss_terminal.label:%s\n",label);
        printf("ss_terminal.result:%d\n",result);
    };
} s_terminal;
...
typedef struct ss_bracket {
    ...
    void show(char* buff) {
          int l;      
          char* my_buf=(char*)malloc(5000);
	      l = this->buf_end-this->buf_start;
	      strncpy(my_buf,buff+buf_start,l);
	      my_buf[l] = '\0';
          printf("whole tree in one line:\n%s\n",buff);
	      printf("ss_bracket.(start=%d,end=%d):%s\n",this->start,this->end,my_buf);
	      printf("ss_bracket.Label:%s\n",label);
          printf("ss_bracket.result:%d\n",result);
          printf("This constituent over!\n\n");
          free(my_buf);
    };
} s_bracket;
...
typedef struct ss_equiv {
    ...
    char *s2;
    void show() {
        printf("ss_equiv.equiv string 1:%s\n",s1);
        printf("ss_equiv.equiv string 2:%s\n",s2);
    };
} s_equiv;
...
void set_param(char* param,char* value);
void pushb(int item);
void calc_result(unsigned char *buf1,unsigned char *buf);
void fix_quote();
void reinsert_term(s_term_ind* quot,s_terminal* terminal,s_bracket* bracket,int * wn);
void preprocess_data();
void massage_data();
void modify_label(char* label);
void individual_result(int wn1,int bn1,int bn2,int match,int crossing,int correct_tag);
void print_total();
void dsp_info();
void Error(char* s,char* arg1=NULL,char* arg2=NULL,char* arg3=NULL,...);
void Fatal(char* s,char* arg1=NULL,char* arg2=NULL,char* arg3=NULL,...);
void Usage();
int narg(char* s);
int read_line(char* buff,s_terminal* terminal, s_term_ind* quotterm, int * wn,s_bracket* bracket,int* bn);
int popb();
int stackempty();
int is_terminator(char c);
int is_deletelabel(char* s);
int is_deletelabel_for_length(char* s);
int is_quote_term(char* s,char* w);
int word_comp(char* s1,char* s2);
int label_comp(char* s1,char* s2);
	...
	if(bracket1[i].result != 5 && 
	  ...
	  if (F_label==0 || label_comp(bracket1[i].label,bracket2[j].label)==1) {
	    ...
	    if (DEBUG>1) {
	      ...
	      my_buf[l] = '\0';
	      //printf("%s\n",my_buf);
	      printf("%s(gold)",my_buf);

	      l = bracket2[i].buf_end-bracket2[i].buf_start;
	      strncpy(my_buf,buf+bracket2[i].buf_start,l);
	      my_buf[l] = '\0';
	      printf("\t%s(test)\n",my_buf);
...
//
//    for (i=0;i<bn1;i++) {
//        bracket1[i].show(buff);
//        }
//
//    printf("MAX_QUOTE_TERM:%d\n",MAX_QUOTE_TERM);
//    printf("Quote_term_n:%d\n",Quote_term_n); 
//    for(i=0;i<MAX_QUOTE_TERM;i++){
//        printf("quotterm1[%d]:%s\n",i,quotterm1[i]);
//    }
...
//
    ...
    printf("end one individual calc\n\n\n");
